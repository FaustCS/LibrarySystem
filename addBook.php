...
if($imageFileType != "png" && $imageFileType != "jpg" && $imageFileType != "jpeg") {
	$err[] =  "Дозволено тільки PNG/JPG/JPEG файли";
}
...
if(count($err)==0) {
    if ($createCatalog->findAuthor($_POST['AuthorFirstName'], $_POST['AuthorSecondName'])) {
		...
		move_uploaded_file($_FILES["FileToUpload"]["tmp_name"], $target_dir . $createCatalog->getLastId() . "." . pathinfo(basename($_FILES["FileToUpload"]["name"]), PATHINFO_EXTENSION));
